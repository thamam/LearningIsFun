function generateFractionQuestion() {
    ...
    switch (type) {
        case 'compare':
            ...
            question = { question: `מה הסימן הנכון? ${num1}/${den1} ___ ${num2}/${den1}`, type: 'choice', choices: choices };
            break;
        case 'addSameDenominator':
            ...
            if (simplified.den === 1) {
                ...
                question = { question: `${n1}/${den} + ${n2}/${den} = ___`, type: 'input' };
            } else if (simplified.num === sum && simplified.den === den) {
                answer = `${sum}/${den}`;
                question = { question: `${n1}/${den} + ${n2}/${den} = ___/${den}`, type: 'input' };
            } else {
                answer = `${simplified.num}/${simplified.den}`;
                question = { question: `${n1}/${den} + ${n2}/${den} = ___ (צמצמי!)`, type: 'input' };
            }
            break;
        case 'simplify':
            ...
            question = { question: `צמצמי: ${numToSimplify}/${denToSimplify} = ___`, type: 'input' };
            break;
        case 'fractionToDecimal':
            ...
            answer = pair.decimal;
            question = { question: `כתבי כעשרוני: ${pair.num}/${pair.den} = ___`, type: 'input' };
            break;
        case 'decimalToFraction':
            ...
            question = { question: `כתבי כשבר: ${decPair.decimal} = ___`, type: 'input' };
            break;
    }
    fractionState.currentQuestion = question;
    fractionState.currentAnswer = answer;
    document.getElementById('fraction-question').textContent = question.question;
...
function selectFractionChoice(choice, button) {
    document.querySelectorAll('#fraction-choice-buttons .choice-btn').forEach(btn => btn.classList.remove('selected'));
...
function checkFractionAnswer() {
    ...
    if (typeof correctAnswer === 'number') {
        ...
        isCorrect = userAnswer.toString().replace(/\s/g, '') === correctAnswer.toString().replace(/\s/g, '');
    }
